...
扩展允许用户配置自己的 API Key、API Base URL、模型名称、目标语言和翻译提示词。首期优先兼容 OpenAI-compatible API，后续再增加 Claude、Gemini 等原生协议。
...
| Markdown 渲染 | react-markdown | 安全展示模型生成的简单格式 |
...
- Markdown 渲染禁用原始 HTML，外部链接进行协议校验。
