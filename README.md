...
- 百度、必应（Microsoft Translator）、Google Cloud Translation v2 官方翻译 API；需要自行配置相应开发者凭据，个人翻译软件订阅不等同于 API 账户。
...
在“翻译服务配置”添加服务：百度填写 App ID 和 Secret（API Key 字段）；Microsoft 填写 Key 与资源需要的 Region；Google 填写 Cloud Translation v2 API Key。保存时按域名授予访问权限，连接测试会实际调用服务。
...
移除 DeepL 服务支持；百度、Google、Microsoft 翻译卡片仅显示服务名称，不再显示“未配置模型”。旧 DeepL 配置的迁移规则见 [更新说明](docs/releases/RELEASE_NOTES_1.3.9.md)。

### 1.3.10 阅读体验

设置页提供连接诊断、译文样式和网站语言/全文模式规则；工具栏可选择网页区域翻译。自动全文翻译会预判目标语言页面，避免无效任务；划词和网页译文支持大模型单次“换一种表达”及撤销。详见 [1.3.10 更新说明](docs/releases/RELEASE_NOTES_1.3.10.md)。

### 1.3.11 服务配置修复

修复机器翻译作为默认服务时无法打开提示词设置、长文本绑定模型后无法切换，以及弹窗机器翻译名称附带“未配置”的问题。统一必应翻译名称，详见 [1.3.11 更新说明](docs/releases/RELEASE_NOTES_1.3.11.md)。

### 1.3.12 数据处理说明

设置页的数据处理说明在勾选同意后自动隐藏，重新打开设置仍保持隐藏；删除并重置后重新显示。详见 [1.3.12 更新说明](docs/releases/RELEASE_NOTES_1.3.12.md)。

### 1.3.13 设置界面修复

数据处理说明同意后收起并保留查看入口；网站规则导出开关归入安全导出卡片；智能输出、输出模式和场景允许随时保存；弹窗移除修改快捷键入口。详见 [1.3.13 更新说明](docs/releases/RELEASE_NOTES_1.3.13.md)。

### 1.3.14 数据处理确认

数据处理说明改为手动点击“了解并同意”，确认后整块消失，不保留折叠入口。详见 [1.3.14 更新说明](docs/releases/RELEASE_NOTES_1.3.14.md)。

### 1.3.15 按服务能力显示设置

弹窗和长文本侧栏按各自绑定服务显示翻译场景；大模型专属选项集中到设置页折叠区域，保留已有偏好。详见 [1.3.15 更新说明](docs/releases/RELEASE_NOTES_1.3.15.md)。

### 1.3.16 分别确认数据处理说明

弹窗与设置页分别记录手动确认，确认一处不会隐藏另一处。详见 [1.3.16 更新说明](docs/releases/RELEASE_NOTES_1.3.16.md)。

### 1.3.17 设置页排版

提示词说明移到页面底部并统一辅助文字样式；增加大模型说明与下方控件的间距。详见 [更新说明](docs/releases/RELEASE_NOTES_1.3.17.md)。

### 1.3.18 功能页面切换服务

弹窗服务选择修复为更新划词实际绑定；全文区域新增服务选择，长文本使用相同的绑定更新规则。详见 [更新说明](docs/releases/RELEASE_NOTES_1.3.18.md)。

### 1.3.19 设置提示精简

移除大模型专属设置中的“当前适用于”功能列表。详见 [更新说明](docs/releases/RELEASE_NOTES_1.3.19.md)。

### 1.3.20 功能精简

移除划词和网页译文的“换一种表达”及配套撤销功能，保留普通重新翻译。详见 [更新说明](docs/releases/RELEASE_NOTES_1.3.20.md)。

### 1.3.21 稳定性与性能

统一跨页面配置保存，细化翻译错误原因，降低空闲查询频率，优化全文采样和排序，并拆分设置组件。新增可重复性能测试与验收工具。详见 [更新说明](docs/releases/RELEASE_NOTES_1.3.21.md)。

### 1.4.0 功能整合版本

整合 1.3 系列的全文与区域翻译、按功能服务绑定、阅读样式、网站规则、服务诊断、隐私确认和稳定性优化，并统一发布版本与工程验证。详见 [更新说明](docs/releases/RELEASE_NOTES_1.4.0.md)。
