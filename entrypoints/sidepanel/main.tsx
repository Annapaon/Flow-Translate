...
function App() {
  ...
  return <main><header><div><span>译</span><div><h1>{t("长文本翻译", "Long text translation")}</h1><p>{t("适合段落、文章与较长内容", "For paragraphs, articles, and long content")}</p></div></div><button onClick={() => browser.tabs.create({ url: browser.runtime.getURL("/options.html") })} title={t("设置", "Settings")}>⚙</button></header>
    ...
    {settings.bidirectional && <p>{t("双向互译：中文 ↔ ", "Bidirectional: Chinese ↔ ")}{settings.pairLanguage}</p>}
    <section className="selectors">
      ...
      <label>{t("翻译模型", "Model")}<select value={settings.activeModelId} onChange={(e) => update({ activeModelId: e.target.value })}>{profiles.map(x => <option value={x.id} key={x.id}>{x.name}</option>)}</select></label>
      <label>{t("翻译场景", "Scene")}<select disabled={isMachine(settings.modelProfiles.find(p=>p.id===settings.activeModelId)?.provider ?? settings.provider)} value={settings.translationScene} onChange={(e) => update({ translationScene: e.target.value as TranslatorSettings["translationScene"] })}>{TRANSLATION_SCENES.map(x => <option value={x.id} key={x.id}>{en ? ({general:"General",technical:"Technical",academic:"Academic",business:"Business"}[x.id]) : x.name}</option>)}</select></label>
    </section>
