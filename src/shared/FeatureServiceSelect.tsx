...
export function FeatureServiceSelect({ settings, feature, label, disabled, onChange }: {
  ...
  return <label>{label}<select aria-label={label} disabled={disabled} value={settings.featureModels[feature]} onChange={event => onChange(event.target.value)}>
    <option value="">{en ? "Follow default service" : "跟随默认服务"}（{profiles.find(profile => profile.id === settings.activeModelId)?.name}）</option>
