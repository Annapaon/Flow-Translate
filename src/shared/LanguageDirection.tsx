...
export function LanguageDirection({ settings: s, update, disabled = false }: { settings: TranslatorSettings; update: (patch: Partial<TranslatorSettings>) => void; disabled?: boolean }) {
  const en = s.uiLanguage === "en";
  const t = (zh: string, english: string) => en ? english : zh;
  return <div className="ft-direction">
    <Toggle label={t("双向互译", "Bidirectional translation")} checked={s.bidirectional} disabled={disabled} onChange={bidirectional => update({ bidirectional })} />
    ...
    {s.bidirectional && <p className="ft-help">{t("自动识别两种语言并互译；其他语言译为语言一。", "Automatically translates between both languages; other languages translate to the first.")}</p>}
  </div>;
}
