...
function App() {
  ...
  return <main><header><div><span>译</span><div><h1>{t("长文本翻译", "Long text translation")}</h1><p>{t("适合段落、文章与较长内容", "For paragraphs, articles, and long content")}</p></div></div><button onClick={() => browser.tabs.create({ url: browser.runtime.getURL("/options.html") })} title={t("设置", "Settings")}>⚙</button></header>
    ...
    <section className="selectors">
      <label>{t("目标语言", "Target language")}<select disabled={settings.bidirectional || running || saving} value={settings.targetLanguage} onChange={(e) => update({ targetLanguage: e.target.value })}>{LANGUAGES.map(x => <option key={x}>{x}</option>)}</select></label>
      ...
      {capabilitiesForFeature(settings, "longText").richOutput && <label>{t("翻译场景", "Scene")}<select disabled={running || saving} value={settings.translationScene} onChange={(e) => update({ translationScene: e.target.value as TranslatorSettings["translationScene"] })}>{TRANSLATION_SCENES.map(x => <option value={x.id} key={x.id}>{en ? ({general:"General",technical:"Technical",academic:"Academic",business:"Business"}[x.id]) : x.name}</option>)}</select></label>}
    </section>
