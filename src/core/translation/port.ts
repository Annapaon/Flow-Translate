...
import { schedule, delay, modelLane } from "./scheduler";
...
export function attachTranslationPort(port: Port) {
  ...
  async function allowed() {
    ...
    if (page && !current.pageTranslationEnabled)
      throw new ServiceError("全文翻译已关闭 / Page translation disabled", false, true);
  ...
  port.onMessage.addListener(async (raw: unknown) => {
    ...
    if (sessionInvalid) {
      send({ type: "error", requestId: message.requestId, fatal: true, message: "会话已暂停，请重新开始 / Session paused; reconnect to continue" });
      return;
    }
    const id = message.requestId;
    if (controllers.size >= (page ? 32 : 8)) {
      send({
        type: "error",
        requestId: id,
    ...
    try {
      ...
      if (message.type === "page-start") {
        ...
        send({
          ...
          maxConcurrency: settings.provider === "baidu" ? 1 : settings.modelProfiles.find(p => p.id === settings!.activeModelId)?.maxConcurrency ?? 2,
          batchSize: settings.provider === "baidu" || settings.enableThinking ? 1 : 4,
      ...
      if (page) {
        ...
        settings = settingsForFeature(settings, feature, message.serviceId);
      ...
      const lane = await modelLane(endpoint.origin, profile);
      const plain =
      ...
      for (const batch of batches) {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
          try {
            ...
            completed = page && batches.length === 1 && batch.length === 1
              ? await batcher.run({ text: chunk, config, signal, html, lane, allowed, persist, single,
                  delta: text => { signal.throwIfAborted(); send({ type: "delta", requestId: id, text }); },
                  reset: () => send({ type: "retry", requestId: id, attempt: 0 }),
                })
              : await single();
            break;
          } catch (error) {
