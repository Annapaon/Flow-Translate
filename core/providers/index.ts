 ...
 */
export async function testConnection(settings: TranslatorSettings): Promise<TestConnectionResponse> {
  try {
    if (isMachine(settings.provider)) {
      await machineTranslate(["Hello"], { ...settings, sourceLanguage: "auto", targetLanguage: "简体中文" }, AbortSignal.timeout(15_000));
      return { ok: true, message: "连接成功 / Connection successful" };
    }
    assertTranslatable(settings);
    const config = toProviderConfig(settings);
    return await getProvider(settings.provider).testConnection(config);
  } catch (error) {
    return {
      ok: false,
