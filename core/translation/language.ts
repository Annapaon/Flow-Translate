...
export async function detect(
  ...
  if (/\p{Script=Han}/u.test(text) && !/[a-z]{4}/i.test(text)) {
    ...
    return { language: "zh-CN", uncertain: text.trim().length < 8 };
  }
...
export async function resolveSettings(
  ...
  if (settings.bidirectional) {
    ...
    target =
      isChinese(source) && !uncertain ? settings.pairLanguage : "简体中文";
    // Explicit source-language hints disambiguate short all-Han selections.
    if (isChinese(source) && hint.startsWith("zh"))
      target = settings.pairLanguage;
  }
