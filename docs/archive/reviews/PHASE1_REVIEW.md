...
| 计划项 | 代码位置 | 状态 |
| --- | --- | --- |
| 普通网页选区 / input / textarea / 跨节点选取 | src/entrypoints/content.tsx:106 readSelection()；光标镜像计算 content.tsx:77 | 完成 |
...
| 模型新增 / 编辑 / 删除，全部配置字段（名称、URL、Key、模型、Headers、Temperature、最大输出、超时） | src/entrypoints/options/main.tsx:448 模型编辑弹窗 | 完成 |
| 测试连接 / 设置默认模型 | options/main.tsx:261 testProfile()、activeModelId 管理 | 完成 |
| OpenAI-compatible Provider | src/core/providers/openai-compatible.ts | 完成 |
| 源语言自动检测 / 目标语言 / 触发模式 / 字符范围 / Prompt / 输出模式 / 场景提示词 / 白黑名单 | options/main.tsx:346-385；内置敏感站点黑名单 src/shared/constants.ts | 完成 |

## 三、P1 / P2 完成情况（计划中已勾选，代码核对属实）

- 多模型快速切换：src/entrypoints/popup/main.tsx（模型/场景/语言快速切换）
- Anthropic 协议 Provider（含 x-api-key / bearer / 双发鉴权）：src/core/providers/anthropic.ts
- Gemini 原生 Provider：src/core/providers/gemini.ts
- Ollama / LM Studio / Xinference / vLLM / SGLang 适配：src/core/providers/registry.ts（统一复用 OpenAI 兼容协议，各自提供默认地址与占位符）
- 翻译历史（查看 / 复制 / 单条删除 / 清空 / 收藏 / 搜索）：src/shared/history.ts + options 页历史分区
- 相同文本结果缓存（SHA-256 键、7 天过期、100 条上限）：src/shared/history.ts
- Side Panel 长文本翻译：src/entrypoints/sidepanel/main.tsx
...
- 按模型使用量统计（请求数、输入输出字符、最后使用、单模型清零）：src/shared/history.ts + options 模型卡片
...
2. 技术栈偏差：已明确并清理。
   ...
   **保留 Zod**：当前 `src/shared/security.ts` 已实际用于配置导入校验，首次报告对其使用情况的描述不准确。
