...
云端模型地址必须使用 HTTPS；只有本机地址（localhost、127.0.0.1、::1）和局域网私有地址（如 192.168.*.*、10.*.*、172.16–172.31.*、169.254.*.* 或 *.local 主机名）可使用 HTTP。保存、测试模型或导入并启用配置时，会通过浏览器申请尚未授予的接口访问权限；导入时可合并申请多个已启用模型的接口。导入文件中的数据处理同意标记不会被信任，本机已确认的同意会保留，首次使用需明确确认。API Key 不会提供给网页内容脚本，错误信息会进行密钥脱敏。扩展不加载或执行远程代码。浏览器本地存储不是系统级密钥保险箱，请勿在不可信设备上保存重要密钥。
...
Cloud endpoints must use HTTPS; HTTP is allowed only for loopback addresses (localhost, 127.0.0.1, ::1) and private LAN addresses (such as 192.168.*.*, 10.*.*, 172.16–172.31.*, 169.254.*.*, or *.local hostnames). When saving, testing, or importing and enabling profiles, the browser requests any missing endpoint permissions; imports can request access for multiple enabled models together. Consent flags in imported files are not trusted. Existing consent on this device is retained; first-time users must explicitly accept the data handling notice. API keys are not exposed to webpage content scripts, errors are redacted, and no remotely hosted code is loaded or executed. Browser local storage is not a system secret vault; do not store important keys on an untrusted device.
