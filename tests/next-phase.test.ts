...
import { resolveSettings } from "../core/translation/language";
import { machineTranslate, ServiceError } from "../core/services/machine";
import { schedule } from "../core/translation/scheduler";
import { getSettings, saveSettings } from "../shared/settings";
import { validateImportedSettings } from "../shared/security";
import {
  createCacheKey,
  cacheTranslation,
...
it("does not fetch when cancelled during asynchronous settings loading", async () => {
  const settingsModule = await import("../shared/settings");
  const { attachTranslationPort } = await import("../core/translation/port");
...
it("normalizes regional hints and matches complete Latin terms", async () => {
  const { languageCode, matchesTerm } =
    await import("../core/translation/language");
