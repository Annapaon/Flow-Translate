...
describe("untrusted provider response security", () => {
  ...
  it("bounds error bodies and cancels oversized streams", async () => {
    const { readLimitedText, MAX_STREAM_BUFFER } = await import("../../src/core/providers/sse");
    let cancelled = false;
  ...
  it("bounds cumulative SSE bytes even when each event is small", async () => {
    const { sseEvents } = await import("../../src/core/providers/sse");
    let cancelled = false;
...
describe("connection test metering", () => {
  it("counts the actual LLM connection request", async () => {
