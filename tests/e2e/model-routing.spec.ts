...
test("feature assignments persist, route requests and keep active page sessions on their original model", async ({ extension: e }) => {
  ...
  const toggle = options.getByRole("switch", { name: "Assign models by feature" });
  await expect(toggle).not.toBeChecked();
  await expect(options.locator(".model-routing select")).toHaveCount(0);
  await toggle.check();
  const selection = options.getByRole("combobox", { name: "Selection translation", exact: true });
  const page = options.getByRole("combobox", { name: "Page translation", exact: true });
  const long = options.getByRole("combobox", { name: "Long text translation", exact: true });
  await selection.selectOption("selection-model");
  await page.selectOption("page-model");
  await long.selectOption("long-model");
  ...
  await options.locator(".model-routing").screenshot({ path: ".output/ui-review/model-routing.png" });
  await toggle.uncheck();
  await expect(options.locator(".model-routing select")).toHaveCount(0);
  await toggle.check();
  await expect(page).toHaveValue("page-model");
  await options.reload();
  await options.getByRole("button", { name: /Models/ }).click();
  await expect(toggle).toBeChecked();
  await expect(long).toHaveValue("long-model");
  ...
  await page.selectOption("long-model");
  ...
  await expect(panel.getByRole("combobox", { name: "Model", exact: true })).toBeDisabled();
...
test("model cards set and persist the default without changing feature assignments", async ({ extension: e }) => {
  ...
  await expect(options.locator(".model-routing-default")).toContainText("New default");
  await expect(options.getByRole("combobox", { name: "Page translation", exact: true })).toHaveValue("default-model");
